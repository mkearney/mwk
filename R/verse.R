core <- c("tfse", "tbltools", "dplyr", "ggplot2", "purrr")
...
mwk_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("mwk ", package_version("mwk"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the mwk and other packages
#'
#' This function lists all the conflicts between packages in the mwk
...
#' @examples
#' mwk_conflicts()
mwk_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", mwk_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "mwk_conflicts")
}

mwk_conflict_message <- function(x) {
...
#' @export
print.mwk_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(mwk_conflict_message(x))
}
...
#' Update mwk packages
#'
#' This will check to see if all mwk packages (and optionally, their
...
#' mwk_update()
#' }
mwk_update <- function(recursive = FALSE) {

  deps <- mwk_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All mwk packages up-to-date")
    return(invisible())
  }
...
#' List all mwk dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   mwk packages.
#' @export
mwk_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("mwk", pkgs, recursive = recursive)
...
#' List all packages in the mwk
#'
#' @param include_self Include mwk in the list?
#' @export
#' @examples
#' mwk_packages()
mwk_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("mwk")$Imports
...
.onAttach <- function(...) {
  ...
  if (!"package:conflicted" %in% search()) {
    x <- mwk_conflicts()
    msg(mwk_conflict_message(x), startup = TRUE)
  }
